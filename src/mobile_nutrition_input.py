...
from .nutrition_logging import MEAL_TYPES, create_meal_record, soft_delete_meal_record
...
def save_mobile_manual_nutrition_entry(
    ...
    amount = _number(payload.get("amount"), "nutrition_amount")
    ...
    try:
        ...
        record_id = create_meal_record(connection, {
            "date": day, "meal_type": meal_type, "eaten_at": eaten_at,
            "status": "completed", "source": "imported", "notes": marker,
        }, [{
            "custom_food_name": food_name, "quantity": amount, "unit": unit,
            "item_type": "food", "user_confirmed": True,
        }])
        connection.execute(
            ...
            (*[nutrients[field] for field in _NUTRIENTS], record_id),
        )
        connection.commit()
    finally:
        connection.close()
